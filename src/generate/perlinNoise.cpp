...
namespace Generate
{
    ...
    {
        std::array<int, 4> corner_index = GetCornerIndex(_x, _y);
    ...
    std::array<glm::vec2, 4> PerlinNoise2D::GetDistanceVectors(std::array<int, 4>& _cornerIndexes, float _x, float _y)
    {
    ...
    std::array<glm::vec2, 4> PerlinNoise2D::GetGradientVectors(std::array<int, 4>& _cornerIndexes)
    {
    ...
    std::array<int, 4> PerlinNoise2D::GetCornerIndex(float _x, float _y)
    {
        int left_index = Utils::Floor(_x / m_gridSize);
        int top_index = Utils::Floor(_y / m_gridSize);
        int right_index = left_index + 1;
        int bottom_index = top_index + 1;

        std::array<int, 4> result = { left_index, right_index, top_index, bottom_index };

        return result;
    }

    glm::vec2 PerlinNoise2D::GetGradientVector(int _x, int _y)
    {
